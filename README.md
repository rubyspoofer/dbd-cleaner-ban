# dbd-cleaner-ban ПОДХОДИТ ТОЛЬКО ДЛЯ MICROSOFT STORE
Клинер для игры Dead by Daylight который поможет обойти бан по железу / бан по лицензии от Microsoft.

**Системные требования:**

ОС: Windows 10/11

Платформа: Microsoft Store (Xbox)

## ВАЖНО
Важно: для полного выхода из забаненого акка МС вас нужно зайти в:

Настройки > Учётные записи > Ваши данные > Отменить автоматический вход во все приложения Майкрософт (сделай учётку локальной)

Далее: 

Настройки > Учётные записи > Электронная почта и учётные записи > Выберите вашу вбаненную учётку и удалите её.

## СПУФЕР ДЛЯ ПОЛНОГО ОБХОДА
Для полного обхода бана вам нужен спуфер, рекомендуем использовать наш приватный RUBY SPOOFER:

**Сайт - https://rubyspoofer.github.io/ruby/**

**Discord - https://discord.gg/btJn66hAuM**

<img width="977" height="509" alt="image" src="https://github.com/user-attachments/assets/95f21e96-2c66-4f6a-9761-3126703a054e" />
